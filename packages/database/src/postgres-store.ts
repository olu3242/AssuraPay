...
export const POSTGRES_TRUST_COLLECTIONS: readonly string[] = Object.freeze([
  ...Object.keys(DEDICATED), ...GOVERNED_DOCUMENTS, ...HISTORY_COLLECTIONS, 'agreementIntakes',
...
export const POSTGRES_ROUTED_TABLES: readonly string[] = Object.freeze([...new Set([
  ...
  ...Object.values(BATCH_L_RELATIONS).map((relation) => relation.table), ...Object.values(BATCH_M_RELATIONS).map((relation) => relation.table), AGREEMENT_INTAKE_RELATION.table,
])].sort());
...
function relationalUpdateTarget(collection:string):RelationalUpdate|undefined { if(isAgreementIntakeCollection(collection))return AGREEMENT_INTAKE_RELATION;
...
export class PostgresTrustStore implements TrustPersistence {
  ...
  private async listScoped<T>(collection:string):Promise<T[]>{const dedicated=DEDICATED[collection];try{
    ...
    if(isAgreementIntakeCollection(collection))return(await AGREEMENT_INTAKE_RELATION.list(this.sql))as unknown as T[];
    ...
    this.requireGoverned(collection);const rows=await this.sql<(StoredRow & {tenant_id:string|null;workspace_id:string|null})[]>`SELECT payload, payload_digest, tenant_id, workspace_id FROM trust_records WHERE collection = ${collection} ORDER BY created_at ASC, record_id ASC`;return rows.map((row)=>{const value=rowToRecord<T>(row);return collection==='legalPolicies'||collection==='legalPolicyVersions'?{...value,tenantId:row.tenant_id,workspaceId:row.workspace_id}:value;});
