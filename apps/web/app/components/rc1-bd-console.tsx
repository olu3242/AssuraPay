...
const workspaceIdOf = (workspace: Workspace) =>
...
export function Rc1BdConsole() {
  ...
  const [pending, setPending] = useState<'VERIFY_EMAIL' | 'LOGIN' | null>(null);
  ...
  const assertionHeaders = useCallback(
    async (workspaceId?: string): Promise<HeadersInit> => {
  ...
  useEffect(() => {
    if (session) void refreshMemberships().catch(() => setMemberships([]));
  ...
  const run = useCallback(
    async (label: string, operation: () => Promise<void>) => {
      ...
      try {
        ...
        setOutcome({
          kind: 'error',
          message: `${label} refused: ${error instanceof Error ? error.message : String(error)}`,
        });
      }
    },
    [],
  ...
  const register = () =>
    run('Registration', async () => {
      ...
      if (!registered.emailVerificationToken) {
        setReference(registered.id);
        setPending('VERIFY_EMAIL');
        return;
      }
  ...
  const signIn = () =>
    run('Sign-in', async () => {
      ...
      if (!challengeResponse.ok)
        throw new Error(await failureOf(challengeResponse));
      const challenge = (await challengeResponse.json()) as {
        challengeId?: string;
        proofToken?: string;
      };
      if (!challenge.challengeId) throw new Error('LOGIN_CHALLENGE_MISSING');
      if (!challenge.proofToken) {
        setReference(challenge.challengeId);
        setPending('LOGIN');
        return;
      }
  ...
  const completeEmailCode = () =>
    run(pending === 'LOGIN' ? 'Sign-in' : 'Email verification', async () => {
      const response = await fetch(
          ...
          : '/api/v1/auth/verify-email',
        {
          ...
          body: JSON.stringify(
            pending === 'LOGIN'
              ? { email, challengeId: reference, proofToken: code }
              : { userId: reference, token: code },
          ),
        },
      );
      if (!response.ok) throw new Error(await failureOf(response));
  ...
  const foundTenant = () =>
    run('Tenant founding', async () => {
      ...
      const found = await fetch(
        `/api/v1/workspaces/${encodeURIComponent(workspaceId)}/found`,
        {
          method: 'POST',
  ...
  const activate = (workspace: Workspace) =>
    run('Context activation', async () => {
      const workspaceId = workspaceIdOf(workspace);
      const response = await fetch(
        `/api/v1/workspaces/${encodeURIComponent(workspaceId)}/activate-context`,
        {
          method: 'POST',
          headers: await assertionHeaders(workspaceId),
          body: JSON.stringify({}),
  ...
  const signOut = () =>
    run('Sign-out', async () => {
      const response = await fetch('/api/v1/auth/logout', {
        method: 'POST',
        headers: await assertionHeaders(session?.activeWorkspaceId),
        body: JSON.stringify({}),
      });
      if (!response.ok) throw new Error(await failureOf(response));
      await refreshSession();
  ...
  return (
    <main className="page-shell" data-testid="bootstrap-console">
      <section className="surface-card">
        ...
        {session?.activeWorkspaceId && <nav aria-label="Next steps"><a href="/agreements/new">Create an agreement</a><a href="/contracts">Manage agreements</a><a href="/performance">Plan performance</a></nav>}
        <p>
          Verify your email, sign in, and choose the workspace where you want to
          work.
        </p>
        ...
        {pending && (
          <form
            onSubmit={(event) => {
              ...
              void completeEmailCode();
            }}
          >
            <h2>
              {pending === 'LOGIN' ? 'Complete sign-in' : 'Verify your email'}
            </h2>
            <p>
              Check your email for the code. Codes expire after ten minutes.
            </p>
            <label>
              Reference
              <input
        ...
        {outcome.kind === 'ok' && (
          <p role="status" data-testid="outcome-ok">
            {outcome.message}
          </p>
        )}
        {outcome.kind === 'error' && (
          <p role="alert" data-testid="outcome-error">
            {outcome.message}
          </p>
        )}

        {!session ? (
          <p data-testid="session-absent">No active session.</p>
        ) : (
          <section data-testid="session-summary">
            <p>
              User: <span data-testid="session-user">{session.userId}</span>
            </p>
            <p>
              Workspace:{' '}
              <span data-testid="session-workspace">
                {session.activeWorkspaceId ?? '— none yet —'}
              </span>
            </p>
            <p>
              Tenant:{' '}
              <span data-testid="session-tenant">
                {session.tenantId ?? '— none yet —'}
              </span>
            </p>
        ...
        {session && (
          <section>
            ...
            <button data-testid="found-tenant" onClick={foundTenant}>
              Found organization
            </button>
            {memberships.length === 0 ? (
              ...
              memberships.map((workspace) => {
                ...
                return (
                  <div data-testid="membership" key={id}>
