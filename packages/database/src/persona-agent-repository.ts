...
type PersonaAgentRow = {
  ...
  allowed_action_classes: unknown;
  max_risk: string;
...
function stringField(record: Row, field: string): string {
  ...
  if (typeof value !== 'string' || value.trim() === '')
    throw new PostgresStoreError('PERSISTENCE_SCHEMA_VIOLATION', `${PERSONA_AGENT_COLLECTION}.${field} is required`);
  return value;
}
...
function optionalString(record: Row, field: string): string | null {
  ...
  if (typeof value !== 'string' || value.trim() === '')
    throw new PostgresStoreError('PERSISTENCE_SCHEMA_VIOLATION', `${PERSONA_AGENT_COLLECTION}.${field} must be a non-empty string when supplied`);
  return value;
}
...
function stringArray(record: Row, field: string): string[] {
  ...
  if (!Array.isArray(value) || value.some((entry) => typeof entry !== 'string' || entry.trim() === ''))
    throw new PostgresStoreError('PERSISTENCE_SCHEMA_VIOLATION', `${PERSONA_AGENT_COLLECTION}.${field} must be a string array`);
...
function integerField(record: Row, field: string): number {
  ...
  if (!Number.isInteger(value))
    throw new PostgresStoreError('PERSISTENCE_SCHEMA_VIOLATION', `${PERSONA_AGENT_COLLECTION}.${field} must be an integer`);
...
function booleanField(record: Row, field: string): boolean {
  ...
  if (typeof value !== 'boolean')
    throw new PostgresStoreError('PERSISTENCE_SCHEMA_VIOLATION', `${PERSONA_AGENT_COLLECTION}.${field} must be boolean`);
  return value;
}
...
function createdAt(record: Row): string {
  ...
  if (Number.isNaN(Date.parse(value)))
    throw new PostgresStoreError('PERSISTENCE_SCHEMA_VIOLATION', `${PERSONA_AGENT_COLLECTION}.createdAt must be an instant`);
  return value;
}
...
export const personaAgentRelation = {
  ...
  async list(sql: SqlClient): Promise<Row[]> {
    ...
    return rows.map((row) => ({
      ...
      allowedActionClasses: row.allowed_action_classes,
  ...
  async insert(sql: SqlClient, record: Row, tenantId: string): Promise<void> {
    ...
    if (carriedTenant !== tenantId)
      throw new PostgresStoreError('PERSISTENCE_SCOPE_INVALID', 'personaAgentProfiles tenant does not match active trust scope');

    const autonomyLevel = integerField(record, 'autonomyLevel');
    if (autonomyLevel < 0 || autonomyLevel > 5)
      throw new PostgresStoreError('PERSISTENCE_SCHEMA_VIOLATION', 'personaAgentProfiles.autonomyLevel must be 0..5');

    const version = integerField(record, 'version');
    if (version < 1)
      throw new PostgresStoreError('PERSISTENCE_SCHEMA_VIOLATION', 'personaAgentProfiles.version must be >= 1');
    ...
    await sql`
      INSERT INTO persona_agent_profiles (
        ...
        ${sql.json(stringArray(record, 'allowedRoles') as never)},
        ${sql.json(stringArray(record, 'allowedActionClasses') as never)}, ${stringField(record, 'maxRisk')},
        ${optionalString(record, 'industry')}, ${optionalString(record, 'organizationId')},
  ...
  async update(sql: SqlClient, record: Row): Promise<number> {
    const rows = await sql<{ id: string }[]>`
      ...
      SET active = ${booleanField(record, 'active')}
      WHERE id = ${stringField(record, 'id')}
      RETURNING id
    `;
    return rows.length;
  },
