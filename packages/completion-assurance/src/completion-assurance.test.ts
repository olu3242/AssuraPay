...
describe('Engine 36 Inspection & Field Verification', () => {
  it('requires full checklist coverage, fails on a missed required item, and only allows reinspection after failure', async () => {
    ...
    await expect(await e.complete(c, { id: inspection.id, findings: [{ checklistItem: 'weld-quality', result: 'FAIL', notes: '' }] })).rejects.toThrow(
    ...
    await expect(await e.schedule(c, { workItemId: 'wi', scheduledFor: '2026-08-12', checklist: [{ item: 'weld-quality', required: true }] })).resolves.not.toThrow();
    ...
    await expect(await e.schedule(c, { workItemId: 'wi2', scheduledFor: '2026-08-11', checklist: [{ item: 'x', required: true }], reinspectionOfId: passedInspection.id })).rejects.toThrow('REINSPECTION_REQUIRES_PRIOR_FAILURE');
...
describe('Engine 37 Issue, Risk & Corrective Action', () => {
  it('routes an issue through escalation, corrective action and resolution, and blocks on open high-severity issues', async () => {
    ...
    await expect(await e.escalate(c, { id: issue.id, reason: '' })).rejects.toThrow('ESCALATION_REASON_REQUIRED');
    ...
    await expect(await e.verifyResolution(c, capa.id)).rejects.toThrow('CAPA_NOT_COMPLETED');
...
describe('Engine 38 Change Control', () => {
  it('requires submission before a decision and approval before implementation', async () => {
    ...
    await expect(await e.decide(c, { changeRequestId: request.id, decision: 'APPROVE', rationale: 'weather' })).rejects.toThrow(
      'CHANGE_REQUEST_NOT_SUBMITTED',
    );
    await e.submit(c, request.id);
    await expect(await e.decide(c, { changeRequestId: request.id, decision: 'APPROVE', rationale: '' })).rejects.toThrow(
      'RATIONALE_REQUIRED',
    );
...
describe('Engine 39 Acceptance & Decision', () => {
  it('requires conditions for a conditional decision and supersedes the prior decision for the same work item', async () => {
    ...
    await expect(await e.decide(c, { workItemId: 'wi', decision: 'CONDITIONAL', rationale: 'minor punch list' })).rejects.toThrow(
      'CONDITIONS_REQUIRED',
    );
...
describe('Engine 40 Completion Certification', () => {
  it('requires every gate to pass before issuing exactly one certificate per work item', async () => {
    ...
    await expect(await e.revoke(c, { id: certificate.id, reason: '' })).rejects.toThrow('REVOCATION_REASON_REQUIRED');
