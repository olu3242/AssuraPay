...
describe('Engine 01 identity trust', () => {
  it('hashes session tokens, audits authentication, revokes reuse, and expires step-up', async () => {
    ...
    const login = await service.login({
      email: user.email,
      rawSessionToken: 'raw-secret-token',
      authenticationMethodId: await loginMethod(store, user.email),
      correlationId: 'c3',
    });
    expect(login.session.sessionTokenHash).not.toContain('raw-secret-token');
    expect(await store.list('auditRecords')).toHaveLength(5);
    ...
    await expect(service.resolveSession('raw-secret-token')).rejects.toThrow(
      'SESSION_INVALID',
    );
  });
  it('denies suspended identities and prevents device trust from changing assurance', async () => {
    const store = new InMemoryTrustStore();
    const service = new IdentityService(store);
    const user = await service.activate(
      (
    ...
    await store.replace('identities', {
      ...user,
...
describe('Engine 01 bulk revocation resolves before it reports', () => {
   ...
   * session stayed live.
   */
  async function activeUserWithSessions(
    ...
    for (let index = 0; index < count; index++)
      await service.login({
        email: user.email,
        rawSessionToken: `token-${index}`,
        authenticationMethodId: await loginMethod(store, user.email),
        correlationId: `login-${index}`,
      });
    return { service, user };
  }
  ...
  it('leaves no session resolvable once it has returned', async () => {
    ...
    for (const index of [0, 1, 2])
      await expect(service.resolveSession(`token-${index}`)).rejects.toThrow(
        'SESSION_INVALID',
      );
  });
  ...
  it('writes an unbroken audit chain, because revocations are appended one at a time', async () => {
    ...
    for (const [index, record] of records.entries())
      expect(record.previousHash).toBe(
        index === 0 ? undefined : records[index - 1].integrityHash,
      );
  });
});
