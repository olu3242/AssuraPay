...
export type TrustedDevice = {
  ...
  displayName: string;
  deviceType: string;
  trustStatus: 'SEEN' | 'TRUSTED' | 'REVOKED';
  firstSeenAt: string;
  lastSeenAt: string;
  trustedAt?: string;
...
export class IdentityService {
  ...
  async register(input: {
    ...
    return this.store.transaction((tx) =>
      new IdentityService(tx).registerInTransaction(input),
    );
  }

  private async registerInTransaction(input: {
    email: string;
    displayName: string;
    correlationId: string;
    verificationTokenTtlMs?: number;
    ...
    if (
      (await this.store.list<UserIdentity>('identities')).some(
        (entry) => entry.email === email && entry.status !== 'DELETED',
      )
    )
    ...
    const identity: UserIdentity = {
      id: randomUUID(),
      tenantNeutralSubjectId: randomUUID(),
      email,
      displayName: input.displayName.trim(),
      status: 'PENDING_VERIFICATION',
      primaryAuthenticationMethod: 'PASSWORDLESS_EMAIL',
      identityAssuranceLevel: 'IAL0_UNVERIFIED',
      emailVerificationTokenHash: verificationTokenDigest(
        emailVerificationToken,
      ),
      emailVerificationExpiresAt: new Date(
        issuedAt.getTime() + (input.verificationTokenTtlMs ?? 10 * 60 * 1000),
      ).toISOString(),
      createdAt: now,
      updatedAt: now,
      version: 1,
    };
    await this.store.append('identities', identity);
    await this.store.audit({
      actorId: identity.id,
      eventType: 'IdentityRegistered',
      aggregateType: 'UserIdentity',
      aggregateId: identity.id,
      correlationId: input.correlationId,
      metadata: { method: 'PASSWORDLESS_EMAIL' },
    });
  ...
  async verifyEmail(input: {
    ...
    try {
      return await this.store.transaction(async (tx) =>
        new IdentityService(tx).verifyEmailInTransaction(input),
      );
    } catch (error) {
      await this.store.audit({
        actorId: 'anonymous',
        eventType: 'IdentityVerificationFailed',
        aggregateType: 'UserIdentity',
      ...
      if (
        ...
        (error.message === 'VERIFICATION_DENIED' ||
          error.message === 'AUTHENTICATION_DENIED' ||
          error.message.includes('PERSISTENCE_DUPLICATE_RECORD'))
      )
        throw new Error('VERIFICATION_DENIED');
      throw error;
    }
  }
  private async verifyEmailInTransaction(input: {
    userId: string;
    token: string;
    correlationId: string;
  }): Promise<UserIdentity> {
    const user = (await this.store.list<UserIdentity>('identities')).find(
      (entry) => entry.id === input.userId,
    );
    ...
    if (
      ...
      !Number.isFinite(Date.parse(user.emailVerificationExpiresAt)) ||
      Date.parse(user.emailVerificationExpiresAt) <= Date.now()
    )
      throw denied();
    if (
      !verificationTokenMatches(input.token, user.emailVerificationTokenHash)
    ) {
      await this.store.audit({
        actorId: user.id,
        eventType: 'IdentityVerificationFailed',
  ...
  async activate(userId: string, correlationId: string) {
    const user = await this.requireIdentity(userId);
    if (user.status !== 'PENDING_VERIFICATION')
      throw new Error('INVALID_IDENTITY_STATE');
    const now = new Date().toISOString(); // The token is cleared here rather than in `verifyEmail`, so it is spent by *any* path that
    ...
    await this.store.replace('identities', updated);
    await this.store.audit({
      actorId: userId,
  ...
  async registerAuthenticationMethod(
    userId: string,
    input: {
      methodType: string;
      provider: string;
    ...
    await this.requireIdentity(userId);
    const now = new Date().toISOString();
    const method: AuthenticationMethod = {
      id: randomUUID(),
      userId,
      ...input,
      providerSubjectReference: hash(input.providerSubjectReference),
      status: 'VERIFIED',
      verifiedAt: now,
      createdAt: now,
      updatedAt: now,
    };
    await this.store.append('authenticationMethods', method);
    return method;
  }
  async login(input: {
    email: string;
    ...
    try {
      return await this.store.transaction(async (tx) =>
        new IdentityService(tx).createSession(input),
      );
    } catch (error) {
      await this.store.audit({
        actorId: 'anonymous',
        ...
        aggregateId: 'unknown',
        correlationId: input.correlationId,
        metadata: { reason: 'POSSESSION_PROOF_REQUIRED_OR_SPENT' },
      });
      if (
        error instanceof Error &&
        (error.message === 'AUTHENTICATION_DENIED' ||
  ...
  private async createSession(input: {
    ...
    const user = (await this.store.list<UserIdentity>('identities')).find(
      (entry) => entry.email === input.email.trim().toLowerCase(),
    );
    if (!user || user.status !== 'ACTIVE') {
      await this.store.audit({
        ...
        eventType: 'AuthenticationFailed',
        aggregateType: 'UserIdentity',
        aggregateId: user?.id ?? 'unknown',
        correlationId: input.correlationId,
        metadata: { reason: 'ACCOUNT_UNAVAILABLE' },
      });
      throw new Error('AUTHENTICATION_DENIED');
    }
    const method = input.authenticationMethodId
      ? (
          await this.store.list<AuthenticationMethod>('authenticationMethods')
        ).find(
          (entry) =>
            entry.id === input.authenticationMethodId &&
    ...
    if (
      !method ||
      method.methodType !== 'PASSWORDLESS_EMAIL_CHALLENGE' ||
      method.provider !== 'assurapay-passwordless-email'
    )
      throw new Error('AUTHENTICATION_DENIED');
    const expiry = (
      method as AuthenticationMethod & { challengeExpiresAt?: string }
    ).challengeExpiresAt;
    if (
      !expiry ||
      ...
      throw new Error('AUTHENTICATION_DENIED');
    if (
      (await this.store.list<UserSession>('sessions')).some(
        (entry) => entry.authenticationMethodId === method.id,
      )
    )
      throw new Error('AUTHENTICATION_DENIED');
    ...
    const session: UserSession = {
      id: methodId,
      userId: user.id,
      sessionTokenHash: hash(input.rawSessionToken),
      authenticationMethodId: methodId,
      identityAssuranceLevel: user.identityAssuranceLevel,
      deviceId,
      ipContext: input.ipContext,
      userAgentContext: input.userAgentContext,
    ...
    await this.store.append('sessions', session);
    await this.store.audit({
      actorId: user.id,
      ...
      correlationId: input.correlationId,
      metadata: { assurance: session.identityAssuranceLevel },
    });
    await this.store.emit({
      aggregateType: 'UserSession',
      aggregateId: session.id,
    ...
    return { session, token: input.rawSessionToken };
  }
  /**
  ...
  async selectWorkspace(input: {
    ...
    const session = (await this.store.list<UserSession>('sessions')).find(
  ...
  async resolveSession(rawToken: string): Promise<UserSession> {
    const tokenHash = hash(rawToken);
    const session = (await this.store.list<UserSession>('sessions')).find(
      (entry) => entry.sessionTokenHash === tokenHash,
    );
    if (
      !session ||
      session.status !== 'ACTIVE' ||
      ...
      throw new Error('SESSION_INVALID');
    const user = await this.requireIdentity(session.userId);
    if (user.status !== 'ACTIVE') throw new Error('IDENTITY_INACTIVE');
    return session;
  }
  async revokeSession(id: string, context: RequestContext, reason: string) {
    const session = (await this.store.list<UserSession>('sessions')).find(
      (entry) => entry.id === id && entry.userId === context.actorUserId,
    );
  ...
  async revokeAll(userId: string, context: RequestContext) {
    const active = (await this.store.list<UserSession>('sessions')).filter(
      (entry) => entry.userId === userId && entry.status === 'ACTIVE',
    );
    const revoked: UserSession[] = [];
    for (const entry of active)
      revoked.push(await this.revokeSession(entry.id, context, 'LOGOUT_ALL'));
    return revoked;
  }
  async registerDevice(
    ...
    const existing = (await this.store.list<TrustedDevice>('devices')).find(
      (entry) =>
        entry.userId === userId &&
        entry.deviceFingerprintHash === fingerprintHash,
    );
    if (existing) {
      const updated = { ...existing, lastSeenAt: now, updatedAt: now };
      await this.store.replace('devices', updated);
      return updated;
    }
    ...
    await this.store.append('devices', device);
    return device;
  }
  async trustDevice(id: string, context: RequestContext) {
    const device = (await this.store.list<TrustedDevice>('devices')).find(
      (entry) => entry.id === id && entry.userId === context.actorUserId,
    );
    ...
    const updated = {
      ...
      trustExpiresAt: new Date(now.getTime() + 30 * 86400000).toISOString(),
  ...
  async requestStepUp(input: {
    ...
    const challenge: StepUpChallenge = {
      ...
      issuedAt: now.toISOString(),
      expiresAt: new Date(
        now.getTime() + (input.ttlMs ?? 300000),
      ).toISOString(),
      createdAt: now.toISOString(),
    };
    await this.store.append('stepUpChallenges', challenge);
    return challenge;
  }
  async completeStepUp(id: string, userId: string) {
    const challenge = (
      await this.store.list<StepUpChallenge>('stepUpChallenges')
    ...
    await this.store.replace('stepUpChallenges', updated);
    return updated;
  }
