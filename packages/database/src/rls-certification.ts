 ...
 *
 * A policy that exists but can be bypassed is not protection, so certification checks
 * enabled+forced RLS and performs live denial probes as a non-owner application role.
 */
...
export async function assertCrossTenantDenied(
  ...
  await sql.begin(async (tx) => {
    ...
    if (crossed.n !== '0') findings.push({ code: 'RLS_CROSS_TENANT_READ', table: 'trust_workspaces', detail: `a caller scoped to ${context.tenantId} read another tenant's workspace` });
    ...
    if (grants.n !== '0') findings.push({ code: 'RLS_CROSS_TENANT_READ', table: 'trust_permission_grants', detail: 'a caller read another tenant’s permission grants' });
    const [audits] = await tx<{ n: string }[]>`SELECT count(*)::text AS n FROM trust_audit_records WHERE workspace_id = ${foreign.workspaceId}`;
    if (audits.n !== '0') findings.push({ code: 'RLS_CROSS_TENANT_READ', table: 'trust_audit_records', detail: 'a caller read another tenant’s audit history' });
    const [profiles] = await tx<{ n: string }[]>`SELECT count(*)::text AS n FROM persona_agent_profiles WHERE workspace_id = ${foreign.workspaceId}`;
    if (profiles.n !== '0') findings.push({ code: 'RLS_CROSS_TENANT_READ', table: 'persona_agent_profiles', detail: 'a caller read another tenant’s persona-agent governance state' });
  });
  return findings;
}
...
export async function assertCrossTenantWriteDenied(
  ...
  if (!refused) findings.push({ code: 'RLS_CROSS_TENANT_WRITE', table: 'trust_records', detail: 'a caller inserted a row attributed to another tenant' });
  return findings;
}
...
export async function assertUnscopedReadDenied(sql: SqlClient, role: string): Promise<RlsFinding[]> {
  ...
  await sql.begin(async (tx) => {
    ...
    for (const table of RLS_GOVERNED_TABLES) {
      const rows = await tx.unsafe<Array<{ n: string }>>(`SELECT count(*)::text AS n FROM "${quoteIdentifier(table)}"`);
...
export async function assertCrossWorkspaceDenied(sql: SqlClient, context: RlsProbeContext, foreignWorkspaceId: string): Promise<RlsFinding[]> {
  const findings: RlsFinding[] = [];
  await sql.begin(async (tx) => {
    await applyProbeRole(tx, context);
    const [profiles] = await tx<{ n: string }[]>`SELECT count(*)::text AS n FROM persona_agent_profiles WHERE workspace_id = ${foreignWorkspaceId}`;
    if (profiles.n !== '0') findings.push({ code: 'RLS_CROSS_WORKSPACE_READ', table: 'persona_agent_profiles', detail: 'a caller read persona profiles from another workspace' });
  });
  return findings;
}
...
export async function assertConnectedRoleCannotBypass(sql: SqlClient): Promise<RlsFinding[]> {
  ...
  `;
  if (row?.rolsuper || row?.rolbypassrls) return [{ code: 'RLS_OWNER_BYPASS', detail: `connected role ${row.current_user} bypasses row-level security` }];
  return [];
}

export async function certifyRowLevelSecurity(sql: SqlClient, schema: string, options: { tables?: readonly string[] } = {}): Promise<RlsCertification> {
  const tables = [...(options.tables ?? RLS_GOVERNED_TABLES)];
  const findings: RlsFinding[] = [];
  const state = await readRlsState(sql, schema);
  for (const table of tables) {
    ...
    else if (!flags.forced) findings.push({ code: 'RLS_NOT_FORCED', table, detail: `${table} does not force row-level security on its owner` });
  ...
  return { certified: findings.length === 0, checkedTables: tables, findings };
}
